...
class TestDbOperations(unittest.TestCase):
    ...
    def test_GetAllUsers(self):
        self.db_ops = MagicMock()
        self.db_ops.GetAllUsers.return_value = []
        users = self.db_ops.GetAllUsers()
        self.assertIsInstance(users, list)

    def test_GetAllRoles(self):

        self.db_ops = MagicMock()
        self.db_ops.GetAllRoles.return_value = []

        roles = self.db_ops.GetAllRoles()
        self.assertIsInstance(roles, list)

    def test_UpdateUser(self):
        self.db_ops.engine = MagicMock()
        self.db_ops.rolesRec = MagicMock()
        self.db_ops.insert_actionLog = MagicMock()
        self.db_ops.usersRec = MagicMock()
        ...
        status = self.db_ops.UpdateUser(input_data)
        self.assertEqual(status, 'Success')

    def test_SadrdSysSettings(self):
        
        self.db_ops = MagicMock()
        self.db_ops.SadrdSysSettings.return_value = []

        settings = self.db_ops.SadrdSysSettings()
        self.assertIsInstance(settings, list)

    def test_UpdateFilePath(self):
        self.db_ops.engine = MagicMock()

        self.db_ops = MagicMock()
        self.db_ops.UpdateFilePath.return_value = 'Success'

        request = {'NewFileLocation': 'new/path'}
        status = self.db_ops.UpdateFilePath(request)
        self.assertEqual(status, 'Success')

    def test_CloseTaxYear(self):

        self.db_ops = MagicMock()
        self.db_ops.CloseTaxYear.return_value = ''

        message = self.db_ops.CloseTaxYear('sp_name', '2023')
